...
List<AdditionalFieldModel> _pagedFields(List<AdditionalFieldModel> allFiltered) {
...
onEntriesChanged: (v) => setState(() {
  ...
  onTap: () {
    final cubit = context.read<AdditionalFieldsCubit>(); // outer, provider-scoped context
    ...
    showDialog(
      ...
      builder: (dialogContext) => LeadSettingsAlert(
        ...
        onSubmit: (String value) async {
          final exists = cubit.state.savedFields
              .any((f) => f.fieldName.toLowerCase() == value.toLowerCase());

          if (exists) {
            _showSnackBar('This field already exists.', isError: true);
            return; // keep dialog open
          }

          Navigator.pop(dialogContext); // close LeadSettingsAlert
          ...
          child: CustomTable(
            ...
            rows: pagedList.asMap().entries.map((entry) {
              ...
              return [
                ...
                Text(field.fieldName, style: AppTextStyle.medium()),
                Center(
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Image.asset(AssetResources.deleteIcon, scale: 1.7),
                    ),
                  ),
                ),
              ];
  ...
  if (totalPages <= 1) {
    return [
      Container(
        ...
        child: Text(
          '1',
          style: AppTextStyle.small(size: 10.sp, color: Colors.white, weight: FontWeight.w600),
        ),
      ),
    ];
  }
    ...
    if (i == 1 || i == totalPages || (i >= _currentPage - 1 && i <= _currentPage + 1)) {
        ...
        MouseRegion(
          ...
          child: GestureDetector(
            ...
            child: Container(
              ...
              margin: const EdgeInsets.symmetric(horizontal: 2),
              decoration: BoxDecoration(
                color: isSelected ? AppThemeColors.appPrimaryColor : Colors.white,
                borderRadius: BorderRadius.circular(6),
                border: Border.all(
                  color: isSelected ? AppThemeColors.appPrimaryColor : const Color(0xFFE2E8F0),
                ),
              ),
              child: Text(
                '$i',
      ...
      pages.add(
        Container(
          ...
          child: Text('...', style: AppTextStyle.small(size: 10.sp, color: const Color(0xFF475569))),
        ),
      );
    }
  }
