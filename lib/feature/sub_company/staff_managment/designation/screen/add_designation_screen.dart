...
class _DesignationPermissionsScreenState
  ...
  bool _hasAnyPermissionSelected() {
  ...
  void _handleSubmit() {
    ...
    if (!_hasAnyPermissionSelected()) {
      _showSnack('Please select at least one permission', isError: true);
      return;
    }
    ...
    return BlocListener<DesignationCubit, DesignationState>(
      listener: (context, state) {
        if (state is DesignationSaved) {
          ...
          final name = _designationController.text.trim();
          isEditMode
              ? _showSnack('Designation "$name" updated!')
  ...
  // ── Designation Field + Submit Row ───────────────────────────────────────

  Widget _buildDesignationAndSubmitRow() {
    return Container(
      ...
      child: Column(
        children: [
          InkWell(
            ...
            child: Container(
              height: 42,
              color: AppThemeColors.followupDateCardBg,
              ...
              child: Row(
                children: [
                  ...
                  _buildSwitch(
                    value: group.selected,
                    onChanged: (_) => _toggleGroupHeader(groupIndex),
                    activeTrackColor: AppThemeColors.tableHeader,
          ...
          children: cols.asMap().entries.map((e) {
            ...
            return Expanded(
              ...
              child: Container(
                width: isMenu ? menuColW : permColW,
                padding: EdgeInsets.symmetric(
                  horizontal: isMenu ? 1.w : 0,
                  vertical: 1.2.h,
                ),
                decoration: BoxDecoration(
                  border: Border(
                    bottom: BorderSide(color: AppThemeColors.textfieldBorder),
                    left: e.key > 0
                        ? BorderSide(color: AppThemeColors.textfieldBorder)
                        : BorderSide.none,
                  ),
                ),
                alignment: isMenu ? Alignment.centerLeft : Alignment.center,
  ...
  //                 value: item.selected,
  //                 onChanged: (_) => _toggleItemRow(groupIndex, itemIndex),
  //                 activeTrackColor: AppThemeColors.statusActive,
  //                 activeThumbColor: Colors.white,
  //                 inactiveTrackColor: AppThemeColors.scaffoldBg,
  //                 inactiveThumbColor: AppThemeColors.switchBorder,
  //               ),
  ...
  //   );
  // }
  Widget _buildTableRow(int groupIndex, int itemIndex, MenuPermission item) {
    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
        border: Border(
      ...
      child: Row(
        children: [
          ...
          ..._permCells(groupIndex, itemIndex, item),
        ],
      ),
    );
  }
  ...
  //       decoration: BoxDecoration(
  //         border: Border(
  //           left: BorderSide(color: AppThemeColors.textfieldBorder),
  //         ),
  //       ),
  //       alignment: Alignment.center,
  //       child: enabled
  ...
  //               value: false,
  //               onChanged: null,
  //               activeTrackColor: AppThemeColors.switchhidecolor,
  //               activeThumbColor: Colors.white,
  ...
  List<Widget> _permCells(int groupIndex, int itemIndex, MenuPermission item) {
    final perms = [
      (item.canCreate, item.createEnabled, 'create'),
      (item.canView, item.viewEnabled, 'view'),
      (item.canEdit, item.editEnabled, 'edit'),
